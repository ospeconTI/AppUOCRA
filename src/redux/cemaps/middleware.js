import { GET, GET_SUCCESS, GET_ERROR, ADD, ADD_SUCCESS, ADD_ERROR, UPDATE, UPDATE_SUCCESS, UPDATE_ERROR, PATCH, PATCH_SUCCESS, PATCH_ERROR, REMOVE, REMOVE_SUCCESS, REMOVE_ERROR, GET_MAPA_LOCALIDAD, GET_MAPA_PROVINCIA, GET_MAPA_TODOS, GET_MAPA_SUCCESS, GET_MAPA_ERROR } from "../cemaps/actions";
...
export const get = ({ dispatch }) => (next) => (action) => {
	next(action);
	if (action.type === GET) {
		let cemaps = require("../../../jsonLocal/cemaps.json");
		dispatch({
			type: GET_SUCCESS,
			payload: {
				send: 1,
...
export const getMapaTodos = ({ dispatch, getState }) => (next) => (action) => {
	next(action);
...
export const getMapaProvincia = ({ dispatch, getState }) => (next) => (action) => {
	next(action);
...
export const getMapaLocalidad = ({ dispatch, getState }) => (next) => (action) => {
	next(action);
