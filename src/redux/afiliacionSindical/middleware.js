import { GET, GET_SUCCESS, GET_ERROR, ADD, ADD_SUCCESS, ADD_ERROR, UPDATE, UPDATE_SUCCESS, UPDATE_ERROR, PATCH, PATCH_SUCCESS, PATCH_ERROR, REMOVE, REMOVE_SUCCESS, REMOVE_ERROR } from "../afiliacionSindical/actions";

import { RESTAdd, RESTDelete, RESTUpdate, RESTPatch } from "../rest/actions";

import { apiRequest } from "../api/actions";
import { fetchJSON } from "../../libs/fetchJSON";
import { afiliacionSindicalFetch } from "../fetchs";
...
export const get =
	...
	(action) => {
		next(action);
		if (action.type === GET) {
			//fetchJSON(dispatch, "afiliacionSindical.json", GET_SUCCESS, GET_ERROR);
		}
	};
...
export const add =
	...
	(action) => {
		next(action);
		if (action.type === ADD) {
			dispatch(RESTAdd(afiliacionSindicalFetch, action.body, ADD_SUCCESS, ADD_ERROR, action.token));
		}
	};
