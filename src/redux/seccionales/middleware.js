import { GET, GET_SUCCESS, GET_ERROR, ADD, ADD_SUCCESS, ADD_ERROR, UPDATE, UPDATE_SUCCESS, UPDATE_ERROR, PATCH, PATCH_SUCCESS, PATCH_ERROR, REMOVE, REMOVE_SUCCESS, REMOVE_ERROR, GET_MAPA_ZONA, GET_MAPA_TODOS, GET_MAPA_SUCCESS, GET_MAPA_ERROR } from "../seccionales/actions";
...
export const get = ({ dispatch }) => (next) => (action) => {
	next(action);
	if (action.type === GET) {
		let seccionales = require("../../../jsonLocal/seccionales.json");
...
export const getMapaZona = ({ dispatch, getState }) => (next) => (action) => {
	next(action);
