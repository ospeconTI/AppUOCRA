import { GET, GET_SUCCESS, GET_ERROR, ADD, ADD_SUCCESS, ADD_ERROR, UPDATE, UPDATE_SUCCESS, UPDATE_ERROR, PATCH, PATCH_SUCCESS, PATCH_ERROR, REMOVE, REMOVE_SUCCESS, REMOVE_ERROR } from "../coberturas/actions";

import { RESTAdd, RESTDelete, RESTUpdate, RESTPatch } from "../rest/actions";

import { apiRequest } from "../api/actions";
import { fetchCobertura } from "../../libs/fetchCobertura";
...
export const get =
	...
	(action) => {
		next(action);
		if (action.type === GET) {
			//			fetchCobertura(dispatch, "https://apis.service.uocra.org/api/ServiciosApp/Afiliados?tipo=" + action.options.tipo + "&docu_nro=" + action.options.documento, GET_SUCCESS, GET_ERROR);
			fetchCobertura(dispatch, "https://apis.service.uocra.org/api/ServiciosApp/Afiliados?tipo=" + action.options.tipo + "&docu_nro=" + action.options.documento, GET_SUCCESS, GET_ERROR);
		}
	};
