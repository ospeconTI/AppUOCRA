...
export const get = ({ dispatch }) => (next) => (action) => {
	next(action);
	if (action.type === GET) {
		fetchCobertura(dispatch, "https://www.uocra.net/OspeconServicios/vAfiliados?tipo=" + action.options.tipo + "&docu_nro=" + action.options.documento, GET_SUCCESS, GET_ERROR);
	}
};
