...
export class cemapsMapaScreen extends connect(store, CEMAPS_DATOS, MAPA_CLICK, MEDIA_CHANGE, SCREEN)(LitElement) {
	constructor() {
		super();
		this.hidden = true;
		this.area = "body";
	...
	render() {
		if (this.centros) {
			return html`
				<div id="cuerpo">
					<div id="titulo">${this.cemapsMapa[this.idioma].titulo}</div>
					<div class="panel">
	...
	stateChanged(state, name) {
		if (name == SCREEN || name == MEDIA_CHANGE) {
			...
			if (haveBodyArea && SeMuestraEnUnasDeEstasPantallas) {
				...
				for (var i = 0; i < this.centros.length; ++i) {
					...
					this.puntos.push([a.longitud, a.latitud, a.nombre, a.direccion, "N"]);
				}
		...
		if (name == CEMAPS_DATOS) {
			this.centros = state.cemaps.entityMapa;
		}
	}
