...
export class gremioConvenioScreen extends connect(store, DATOS_CONVENIO, ERROR_CONVENIO, MEDIA_CHANGE, SCREEN)(LitElement) {
		...
		if (this.convenios) {
			return html`
				<div id="cuerpo" class="grid row">
					...
					<div id="subTituloTexto">Descargá los <b>Convenios Colectivos de Trabajo</b> vigentes en el ámbito de la industria de la construcción.</div>
			...
			if (this.current == "gremioConvenio" && this.convenios == 0) {
	...
	stateChanged(state, name) {
		...
		if (name == DATOS_CONVENIO) {
			this.convenios = state.convenios.entities.sort(function (a, b) {
