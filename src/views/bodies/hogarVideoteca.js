...
const HOGARVIDEOS_ERROR = "hogarVideos.errorTimeStamp";

export class hogarVideotecaScreen extends connect(store, HOGARVIDEOS_DATOS, HOGARVIDEOS_ERROR, MEDIA_CHANGE, SCREEN)(LitElement) {
	constructor() {
		super();
		this.hidden = true;
		this.area = "body";
	...
	render() {
		if (this.registros) {
			...
			if (this.current == "hogarVideoteca" && this.registros == 0) {
	...
	stateChanged(state, name) {
		...
		if (name == HOGARVIDEOS_DATOS) {
			this.registros = state.hogarVideos.entities.filter((a) => a.activo == 1);
			this.registros = this.registros.sort(function (a, b) {
				return a["orden"] > b["orden"] ? 1 : -1;
			});
			this.update();
		}
		if (name == HOGARVIDEOS_ERROR) {
			this.registros = 0;
			this.update();
		}
	}
